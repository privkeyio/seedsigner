...
class TestMixedSighashFlow(FlowTest):
    """A PSBT whose inputs declare hash types no single label covers.

    Input 0 asks for the unified message, input 1 for the standard one. Both are types
    this device signs, so PSBTParser admits the transaction and both inputs really do
    get signed, under two different messages. Nothing on the review screens says so, and
    the signature count reaches the input count, so without a check this reviews and
    reports exactly like an ordinary transaction.
    """
    ...
    def test_a_transaction_no_single_label_describes_is_refused(self):
        def load_psbt_into_decoder(view: scan_views.ScanView):
    ...
    def test_signing_it_covers_two_different_messages(self):
        """The hazard the refusal exists for. Both inputs sign and the count reaches the
        input count, which is what would report success, so demonstrate it rather than
        ...
        assert [inp.sighash_type for inp in psbt.inputs] == [
            SIGHASH.UNIFIED | SIGHASH.ALL, SIGHASH.ALL
        ]
        assert PSBTParser.unsignable_inputs(psbt) == [], \
            "nothing is skipped here; the refusal is about what the screen could say"

        psbt.sign_with(_root(), sighash=PSBTParser.sighash_type(psbt))

        assert PSBTParser.sig_count(psbt) == len(psbt.inputs), \
            "the count reaches the input count, which is what would report success"
        assert {bytes(sig)[-1] for inp in psbt.inputs for sig in inp.partial_sigs.values()} == {
            SIGHASH.UNIFIED | SIGHASH.ALL, SIGHASH.ALL
        }, "the two signatures cover different messages, and no one label names both"
...
class TestSigningRaisesAfterApproval(FlowTest):
    ...
    def test_this_fixture_never_reaches_the_finalize_view(self):
        ...
        with pytest.raises(InvalidPSBTError) as refused:
            PSBTParser(PSBT.parse(b64decode(self.RAISING_PSBT)), seed=_seed(),
                       network=SettingsConstants.MAINNET)
        assert refused.value.code == RejectCode.MISSING_UTXO
...
class TestAPsbtThatParsesAndThenRaises(FlowTest):
    """The case the copy exists for.

    Input 1 keeps this seed's fingerprint but names a derivation whose key is not the
    pubkey the PSBT gives, so embit derives, compares, and refuses while signing rather
    than while parsing: input 0 is already signed in place when it does.

    This fork closes that door earlier, refusing the same PSBT at load, so the flow
    never reaches signing with it. The guard in the view is still what stands between a
    signing failure and a half-signed object on the controller, for any PSBT that gets
    past the parser and fails later, so it is exercised here by making the signing call
    itself raise.
    """
    ...
    def test_the_parser_refuses_it_before_signing_is_reached(self):
        """Where the fork stops this one, so the class below is not read as covering a
        flow it cannot reach."""
        from seedsigner.models.psbt_parser import InvalidPSBTError, RejectCode

        with pytest.raises(InvalidPSBTError) as refused:
            self._load(self.PSBT_B64)
        assert refused.value.code == RejectCode.FORGED_INPUT_OWNERSHIP
    ...
    def test_the_view_refuses_and_leaves_the_scanned_psbt_alone(self):
        """A PSBT this fork does admit, whose signing then fails. The signature it makes
        before failing lands on the copy, so what the controller holds is still what was
        scanned."""
        from base64 import b64decode
        from unittest.mock import patch
        ...
        controller = self._load(TestUnifiedSighashFlow.UNIFIED_PSBT)
        raw = b64decode(TestUnifiedSighashFlow.UNIFIED_PSBT)

        signed_by_the_view = []
        real_sign_with = PSBT.sign_with

        def sign_then_raise(self, root, sighash=0):
            real_sign_with(self, root, sighash=sighash)
            signed_by_the_view.append(self)
            raise PSBTError("Derivation path is not valid")

        with patch("seedsigner.views.view.View.run_screen") as run_screen:
            run_screen.return_value = 0  # the user approves
            with patch.object(PSBT, "sign_with", sign_then_raise):
                destination = psbt_views.PSBTFinalizeView().run()

        assert len(signed_by_the_view) == 1, "the view never reached signing"
        assert PSBTParser.sig_count(signed_by_the_view[0]) > 0, \
            "nothing was signed before the failure, so the assertions below are vacuous"
        assert signed_by_the_view[0] is not controller.psbt, "the view signed in place"
