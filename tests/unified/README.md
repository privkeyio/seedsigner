...
It also runs `sign_psbt_with_satochip` against a card that holds the same keys
and signs whatever 32 bytes it is handed, which is all a real applet does with
the digest. That covers the part of card signing that is not on the card: the
message the digest is built from and the byte appended to the signature. It is
not a substitute for running the flow on a card.
